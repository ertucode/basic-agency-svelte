<script>
	...
	const lis = ['work', 'about', 'news', 'thinking', 'careers', 'contact'];
</script>

<header class="fixed inset-x-0 top-0 py-12 px-16 z-50">
	<nav class="text-white flex justify-between items-end">
		<div>
			<figure class="w-32 h-4 xl:w-40 xl:h-6">
				<svg
				...
				>
					...
					/>
					<path
						d="M204 17.2h-1.4v1.6h1.4c.5 0 .7-.3.7-.8s-.3-.8-.7-.8zm-.1-.8c1.2 0 1.8.5 1.8 1.6 0 .6-.3 1.1-.9 1.3l1.2 1.8h-1.2l-1.1-1.6h-1.2v1.6h-1.1v-4.7h2.5z"
					/>
				</svg>
			</figure>
		</div>
		<ul class="flex gap-20 list-none uppercase leading-4 text-lg">
			{#each lis as l}
				<li>{l}</li>
			{/each}
		</ul>
